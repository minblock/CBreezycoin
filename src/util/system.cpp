...
const char * const BITCOIN_CONF_FILENAME = "cbreezycoin.conf";
...
#else
    const char* pszModule = "cbreezycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Cbreezycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Cbreezycoin";
#else
    // Unix
    return pathRet / ".cbreezycoin";
#endif
#endif
}
